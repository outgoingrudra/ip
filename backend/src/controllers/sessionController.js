...
export async function createSession(req,res){
    try {
       ...
       const channel =  chatClient.channel("messaging",callId,{
        ...
        //todo send email when session created 
 
    } catch (error) {
        console.log("Error in session Controller.js"+Error);
...
export async function getMyRecentSessions(req,res){
    try {
        ...
        res.status(200).json({
         ...
         console.log("Error in getMyRecentSessions Controller.js"+Error);
         res.status(500).json({ message : "Internal Server Error"})
...
export async function getSessionById(req,res){
    try {
        ...
        res.status(200).json({session})
          ...
          console.log("Error in getSessionById Controller.js"+Error);
         res.status(500).json({ message : "Internal Server Error"})
...
export async function joinSession(req,res){
    try {
        ...
        res.status(200).json({session})
         ...
         console.log("Error in join session Controller.js"+Error);
         res.status(500).json({ message : "Internal Server Error"})
    }
}
...
export async function endSession(req,res){
    try {
        ...
        res.status(200).json({
         ...
         console.log("Error in end session Controller.js"+Error);
         res.status(500).json({ message : "Internal Server Error"})
